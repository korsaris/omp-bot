...
import (
	...
	"github.com/ozonmp/omp-bot/internal/app/commands/storage"
...
func NewRouter(
	...
	return &Router{
		...
		// payment
		// storage
		storageCommander: storage.NewStorageCommander(bot),
		// streaming
		// business
		// work
		// service
...
func (c *Router) handleCallback(callback *tgbotapi.CallbackQuery) {
	...
	case "payment":
		break
	case "storage":
		c.storageCommander.HandleCallback(callback, callbackPath)
		break
	case "streaming":
		break
	case "business":
...
func (c *Router) handleMessage(msg *tgbotapi.Message) {
	...
	case "payment":
		break
	case "storage":
		c.storageCommander.HandleCommand(msg, commandPath)
		break
	case "streaming":
		break
	case "business":
